...
// These properties are dummy values, why query each individually and not all at
// once like qiskit properties?
...
// hardcoded coupling map as for each qubit, what are the two neighbours. Are
// QDMI qubits 1 indexed? Assuming yes and full connectivity for chip.
int QDMI_set_coupling_mapping(QDMI_Device dev, int qubit_index,
                              QDMI_Qubit qubit) {
  ...
  switch (qubit_index) {
  case 0:
    qubit->coupling_mapping =
        (QDMI_qubit_index *)malloc(4 * sizeof(QDMI_qubit_index));
...
// Looks like this is initializing qubits and setting the coupling map for each
// qubit
...
int QDMI_device_status(QDMI_Device dev, QInfo info, int *status) {
  ...
  curl_easy_setopt(curl, CURLOPT_URL,
                   "https://wmiqc-api.wmi.badw.de/1/wmiqc/qobj");
  ...
  curl_easy_setopt(
      ...
      "{\"qobj\": {\"header\": {\"backend_name\": \"dedicated\"}}}");
...
int QDMI_control_test(QDMI_Device dev, QDMI_Job *job, int *flag,
  ...
  // err = QDMI_control_readout_size(dev, status, &numbits); // TODO Why do we
  // need this? CHECK_ERR(err, "QDMI_control_readout_raw_num");
