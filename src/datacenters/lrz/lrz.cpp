...
int LRZ_QDMI_device_session_query_site_property(LRZ_QDMI_Device_Session session,
  ...
  ADD_SINGLE_VALUE_PROPERTY(QDMI_SITE_PROPERTY_INDEX, uint64_t, site->index,
...
int LRZ_QDMI_device_session_query_operation_property(
  ...
  ADD_STRING_PROPERTY(QDMI_OPERATION_PROPERTY_NAME, operation->mName.c_str(),
                      prop, size, value, size_ret)

  ADD_SINGLE_VALUE_PROPERTY(QDMI_OPERATION_PROPERTY_QUBITSNUM, size_t,
                            operation->mQubitNumber, prop, size, value,
                            size_ret)
  ADD_SINGLE_VALUE_PROPERTY(QDMI_OPERATION_PROPERTY_PARAMETERSNUM, size_t,
                            operation->mParameterNumber, prop, size, value,
                            size_ret)
  ADD_LIST_PROPERTY(QDMI_OPERATION_PROPERTY_SITES, LRZ_QDMI_Site,
                    operation->mSupportedSites, prop, size, value, size_ret)
...
int LRZ_QDMI_device_session_create_device_job(LRZ_QDMI_Device_Session session,
  ...
  *job = new LRZ_QDMI_Device_Job_impl_d();
  (*job)->session = session;
...
int LRZ_QDMI_device_job_set_parameter(LRZ_QDMI_Device_Job job,
  ...
  if (param == QDMI_DEVICE_JOB_PARAMETER_PROGRAM) {
    const char *program = static_cast<const char *>(value);
    job->circuit.assign(program, size - 1);
    job->circuit[size] = '\0';
    return QDMI_SUCCESS;
  }
...
void LRZ_QDMI_device_job_free(LRZ_QDMI_Device_Job job) { delete job; }
...
int LRZ_QDMI_device_job_submit(LRZ_QDMI_Device_Job job) {
  ...
  auto job_request =
      CircuitJobRequest(job->circuit, circuit_format,
                        job->session->getResource(), job->shots, 0, 0);
...
int LRZ_QDMI_device_job_wait(LRZ_QDMI_Device_Job job, const size_t timeout) {
  ...
  auto results = job->job_result->getResults();
  std::map<std::string, unsigned int> result = results[0];
  for (auto _count : result) {
...
int LRZ_QDMI_device_job_get_results(LRZ_QDMI_Device_Job job,
  ...
  if (job == NULL || (result != QDMI_JOB_RESULT_HIST_KEYS &&
  ...
  if (job->job_result == nullptr) {
    ...
    if (job->job_result == nullptr)
      return QDMI_ERROR_INVALIDARGUMENT;
    auto results = job->job_result->getResults();
    std::map<std::string, unsigned int> counts = results[0];
